                        ...
                        <table class="min-w-full border divide-y divide-gray-200">
                            ...
                            <tbody wire:sortable="updateOrder" class="bg-white divide-y divide-gray-200 divide-solid">
                                @foreach($categories as $category)
                                    <tr wire:sortable.item="{{ $category->id }}" wire:key="{{ $loop->index }}" class="bg-white">
                                        <td class="px-6">
                                        ...
                                        <td class="px-6">
                                            <div class="inline-block relative mr-2 w-10 align-middle transition duration-200 ease-in select-none">
                                                <input wire:model="active.{{ $category->id }}" wire:click="toggleIsActive({{ $category->id }})" type="checkbox" name="toggle" id="{{ $loop->index.$category->id }}" class="block absolute w-6 h-6 bg-white rounded-full border-4 appearance-none cursor-pointer focus:outline-none toggle-checkbox transition duration-700" />
                                                <label for="{{ $loop->index.$category->id }}" class="block overflow-hidden h-6 bg-gray-300 rounded-full cursor-pointer toggle-label transition duration-500"></label>
                                            </div>
                                        </td>
            ...
            <form wire:submit.prevent="save" class="w-full">
                <div class="flex flex-col items-start p-4">
                    ...
                    <div class="mb-2 w-full">
                        ...
                        <input wire:model.debounce.1000ms="category.name" id="category.name"
